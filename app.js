...
function setupEventListeners() {
    ...
    // AI Generate Button in Upload Form
    DOM.btnAIGenerate.addEventListener('click', handleAIGenerateInForm);
    ...
    DOM.aiChatOverlay.addEventListener('click', closeAIChat);
    DOM.aiChatCloseBtn.addEventListener('click', closeAIChat);
...
async function handleTestAIConnection() {
    ...
    DOM.aiConnStatus.style.display = 'block';
    DOM.aiConnStatus.querySelector('.status-msg').textContent = 'Verificando...';
    ...
    try {
        ...
        DOM.aiConnStatus.querySelector('.status-msg').style.color = 'var(--color-sucu)';
    } catch (err) {
        console.error('AI connection test failed:', err);
        DOM.aiConnStatus.querySelector('.status-msg').textContent = `✗ Error: ${err.message}`;
        DOM.aiConnStatus.querySelector('.status-msg').style.color = 'var(--accent-red)';
    }
}
...
async function handleSaveAISettings(e) {
    ...
    try {
        AIAgentManager.saveApiKey(apiKey);
...
function openAIChat() {
    ...
    DOM.aiChatContentWrapper = DOM.aiChatModal.querySelector('.ai-chat-content-wrapper');
    DOM.aiChatContentWrapper.classList.add('active');
    
    // Initialize AI Agent and update status
    AIAgentManager.init();
    updateAIChatStatus();
...
function closeAIChat() {
    ...
    DOM.aiChatContentWrapper = DOM.aiChatModal.querySelector('.ai-chat-content-wrapper');
    DOM.aiChatContentWrapper.classList.remove('active');
}

// Update AI Chat Status
function updateAIChatStatus() {
    const statusText = DOM.aiChatStatus.querySelector('.status-text');
    if (AIAgentManager.isConfigured) {
        DOM.aiChatStatus.classList.add('connected');
        statusText.textContent = 'Conectado';
    } else {
        DOM.aiChatStatus.classList.remove('connected');
...
async function handleChatSubmit(e) {
    ...
    DOM.aiChatSend.disabled = true;

    try {
        // Get inventory context
        const context = {
            inventoryData: AIAgentManager.formatInventoryForAI(state.items)
        };

        // Call AI Agent
        const response = await AIAgentManager.chat(message, context);
...
async function handleQuickAction(e) {
    ...
    switch (action) {
        case 'analyze':
            await handleAnalyzeInventory();
            break;
        case 'generate-flyer':
            await handleGenerateFlyer();
            break;
        case 'generate-image':
...
// Handle Analyze Inventory Action
async function handleAnalyzeInventory() {
    addChatMessage('user', 'Analiza mi inventario y dame recomendaciones');
    
    try {
        const response = await AIAgentManager.analyzeInventory(state.items);
        addChatMessage('ai', response.message);
    } catch (err) {
...
// Handle Generate Flyer Action
async function handleGenerateFlyer() {
    const category = prompt('¿Para qué categoría quieres generar el flyer? (suculentas/pc/web)');
    if (!category) return;
    
    const productName = prompt('¿Cuál es el producto o servicio?');
    if (!productName) return;
    
    const additionalInfo = prompt('¿Información adicional? (opcional)', '') || '';
    
    addChatMessage('user', `Generar contenido de flyer para ${category} - ${productName}`);
    
    try {
        const response = await AIAgentManager.generateFlyerContent(category, productName, additionalInfo);
        addChatMessage('ai', response.message);
    } catch (err) {
...
async function handleGenerateImage() {
    const prompt = prompt('Describe la imagen que quieres generar:');
    if (!prompt) return;
    
    const category = prompt('¿Categoría? (suculentas/pc/web/general)', 'general') || 'general';
    
    addChatMessage('user', `Generar imagen: ${prompt}`);
    
    try {
        const response = await AIAgentManager.generateImage(prompt, category);
        
        // Add success message
        addChatMessage('ai', `✓ Imagen generada exitosamente. Prompt revisado: "${response.revisedPrompt}"`);
        ...
        // Add image to chat
        addChatMessage('ai', `<img src="${imageUrl}" alt="Generated image" style="max-width: 100%; border-radius: 8px; margin-top: 8px;">`, true);
        
        // Offer to save to inventory
        addChatMessage('ai', '¿Quieres guardar esta imagen en tu inventario? Si es así, usa el formulario de subida con la imagen descargada.');
    } catch (err) {
...
function addChatMessage(type, content, isHTML = false) {
    ...
    avatarDiv.innerHTML = type === 'ai' ? '<i class="fa-solid fa-robot"></i>' : '<i class="fa-solid fa-user"></i>';
    ...
    DOM.aiChatMessages.appendChild(messageDiv);
...
async function handleAIGenerateInForm() {
    ...
    const prompt = prompt('Describe la imagen que quieres generar para tu inventario:');
    if (!prompt) return;

    try {
        DOM.btnAIGenerate.disabled = true;
        DOM.btnAIGenerate.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Generando...';
        ...
        if (!title) {
            DOM.inputTitle.value = prompt.substring(0, 50);
        }
        
        // Auto-fill description if empty
        if (!description) {
            DOM.inputDescription.value = `Imagen generada con IA. Prompt: "${prompt}"`;
        }

        alert('¡Imagen generada exitosamente! Ya puedes guardarla en tu inventario.');
    } catch (err) {
        console.error('AI generation error:', err);
        alert(`Error al generar imagen: ${err.message}`);
    } finally {
        DOM.btnAIGenerate.disabled = false;
        DOM.btnAIGenerate.innerHTML = '<i class="fa-solid fa-wand-magic-sparkles"></i> Generar con IA';
    }
}
